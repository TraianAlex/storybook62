...
const Filter = ({ cards, selectCard, cardsFiltered, cardsFilterSelect }) => {
  const card = cards
    .filter(({ type }) => {
      return !cardsFiltered || type === cardsFiltered;
    })
    .map((card) => {
      return (
  ...
  const filterButtons = FILTERS.map((filterName) => {
    return (
      <Button key={filterName} onClick={() => cardsFilterSelect(filterName)}>
        {filterName}
      </Button>
    );
  });
